**ARCHIVED**: aplnb has moved into [bAsedPL](https://github.com/AnswerDotAI/basedpl), and this repository is archived. The `%apl` and `%%apl` magics are now in `basedpl.notebooks`, and the Dyalog session API is now `basedpl.dyalog`.
